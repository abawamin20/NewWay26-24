...
class PagesService {
  ...
  getFilteredPages = async (
    ...
    <And>
      ...
      // Iterate through the filters to convert them into CAML
      filters.forEach((filter) => {
        if (filter.values.length > 0) {
          switch (filter.filterColumnType) {
            case "DateTime":
              const dateFilters = filter.values
                .map((value) => {
                  ...
                  const endDate = new Date(value);
                  endDate.setDate(endDate.getDate() + 1); // Add 1 day for "less than" comparison
            ...
            <And>
              ...
              filterQuery = `<And>${filterQuery}${dateFilters}</And>`;
              break;
            ...
            <Eq>
              ...
              filterQuery = `<And>${filterQuery}${userFilters}</And>`;
              break;
            ...
            <Eq>
              ...
              filterQuery = `<And>${filterQuery}${urlFilters}</And>`;
              break;

            case "Number":
              const numberFilters = filter.values
                .map((value) => {
                  return `
            <Eq>
              <FieldRef Name="${filter.filterColumn}" />
              ...
              filterQuery = `<And>${filterQuery}${numberFilters}</And>`;
              break;
            ...
            <Eq>
              ...
              filterQuery = `<And>${filterQuery}${columnFilters}</And>`;
              break;
          }
        }
      });
      if (searchText) {
        const isNumber = !isNaN(parseInt(searchText, 10));
      ...
      <Eq>
        ...
        <Value Type="Number">${parseInt(searchText, 10)}</Value>
      </Eq>
    </And>`;
        }
      }
  ...
  <View Scope="RecursiveAll">
    ...
    <RowLimit>${pagesSize}</RowLimit>
  </View>`;

      // Build query parameters (paging needs to be included as a query string)
      let queryParams: {
        ViewXml: string;
      ...
      // If there is a page index for paging, append the paging information
      if (pageIndex > 0) {
        queryParams.Paging = `Paged=TRUE&p_ID=${pageIndex * pagesSize}`;
      }

      // Call RenderListDataAsStream API to fetch items
      const result = await this._sp.web.lists
        .getByTitle("Site Pages")
        .renderListDataAsStream(queryParams);

      // Extract rows from the result
      const rows = result.Row;

      // Return the fetched items (rows) for this request
      return { pages: rows, fetchednextPageUrl: result.NextHref };
    } catch (error) {
